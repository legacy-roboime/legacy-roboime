...
void Simulation::cloneRobot(int indexRobot, int indexScene, string robotLabel, NxMat34 pose)
{
	//Veiculo descricao
	//Body Descricao
	NxActor* robotModel = Simulation::getActorRobot(indexScene, indexRobot);
	NxActor* robotActor = Simulation::cloneActor(robotModel);
	//NxActor* robotActor = Simulation::getActorRobot(indexScene, indexRobot);
	NxShape*const* robotShapes = robotActor->getShapes();
	//NxBounds3 bodyBounds;
	//robotShapes[0]->getWorldBounds(bodyBounds);
