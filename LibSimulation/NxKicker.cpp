...
void NxKicker::controlKicker( float kickerSpeed, NxRobot* robot )
{
	...
	if(robot && kickerSpeed>0){
		...
		if(gUtilLib->NxSweepBoxSphere(box, sphere, -dir, 100, min_dist, normal)){ // Nao sei pq tem q ser o vetor -dir pra da certo pela logica eh NxVec3(cos(angle), sin(angle), 0)
			ball->ball->addForce(/*NxVec3(kickerSpeed*cos(angle)*150., kickerSpeed*sin(angle)*150., 0)*/(-normal)*kickerSpeed*150., NX_IMPULSE); //TODO: VERIFICAR A FORÇA OU IMPULSO
		}
	}
}
